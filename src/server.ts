...
export async function startMcpServer(config: ProbesConfig): Promise<void> {
  ...
  server.registerTool(
    "record_begin",
    {
      description: "Start a new proof record entry for a test.",
      inputSchema: {
        test_name: z.string().describe("Full test name including describe block"),
      },
    },
    async ({ test_name }) => {
      instance.record.begin({ test_name });
      return {
        content: [{ type: "text" as const, text: `Record started: ${test_name}` }],
      };
    }
  );
  ...
  server.registerTool(
    "record_end",
    {
      ...
      inputSchema: {
        result: z.enum(["pass", "fail"]).describe("Test result"),
        error: z.string().optional().describe("Error message if result is fail"),
      },
    },
    async ({ result, error }) => {
      instance.record.end({ result, error });
      return {
        content: [{ type: "text" as const, text: `Record ended: ${result}` }],
      };
    }
  );
  ...
  server.registerTool(
    ...
    async () => {
      await instance.record.write();
      return {
        content: [{ type: "text" as const, text: "Proof records written" }],
      };
    }
  );
